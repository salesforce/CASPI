...
from caffe2.proto.caffe2_legacy_pb2 import VALID
...
class Model(object):
    ...
    def validate(self, data='dev', do_test=False):
        ...
        for batch_num, dial_batch in enumerate(data_iterator):
            py_prev = {'bspn': None};print(batch_num);print('dial_batch:',len(dial_batch))
            for turn_num, turn_batch in enumerate(dial_batch):
                first_turn = (turn_num==0);print('turn_num:',turn_num)
                inputs = self.reader.convert_batch(turn_batch, py_prev, first_turn=first_turn, dst_start_token=self.model.config.decoder_start_token_id)
                for k in inputs:
                    if k!="turn_domain":
                        inputs[k] = inputs[k].to(self.args.device)
...
def main():
    ...
    parser.add_argument("--caspi_returns_file", default='')
    parser.add_argument("--caspi", action='store_true', default=False)
    parser.add_argument("--caspi_wt", default=1.0)
    parser.add_argument("--caspi_val_fraction", default=0.1)
    parser.add_argument("--caspi_data_file", default='data_for_damd.json')
    args = parser.parse_args()
    
    #CASPI param starts
    global RETURNS_PATH,ADOPT_CASPI,CASPI_WT,CASPI_DATA_FILE
    ...
    print('RETURNS_PATH:',RETURNS_PATH)
    print('ADOPT_CASPI:',ADOPT_CASPI)
    print('CASPI_WT:',CASPI_WT)
    ...
    if args.mode == 'test' or args.mode == 'relex':
        ...
        parse_arg_cfg(args)
        if args.model_path=="":
            args.model_path = 'experiments/{}_sd{}_lr{}_bs{}_sp{}_dc{}_cw{}_model_{}_noupdate{}_{}_caspi_{}/'.format('-'.join(cfg.exp_domains), cfg.seed, args.lr, cfg.batch_size,
                                                                                            cfg.early_stop_count, args.lr_decay, args.context_window, args.pretrained_checkpoint, args.noupdate_dst, args.fraction, caspi_uid)
